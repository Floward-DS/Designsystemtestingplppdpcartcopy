...
export function HomeHeader({ 
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 max-w-md mx-auto pointer-events-none">
      <div 
      ...
      >
        <div className="relative z-10 flex items-center justify-between px-[21px] pr-[16px] pt-[48px] pointer-events-auto">
          {/* Floward Logo - changes based on scroll state with animation */}
          <div className="relative w-[140px] h-[50px]">
            {/* Alternative logo when scrolled */}
            <img 
              src="https://raw.githubusercontent.com/Floward-DS/Designsystemtestingplppdpcartcopy/main/src/assets/logo-alt.svg" 
              alt="Floward" 
              className={`absolute top-0 left-0 w-[100px] h-[50px] object-contain object-left transition-all duration-500 ease-in-out ${
                isScrolled 
                  ? 'opacity-100 scale-100 translate-y-0' 
                  : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'
              }`}
              onError={(e) => {
                console.error('Failed to load logo-alt.svg');
            ...
            <svg 
              ...
              className={`absolute top-1/2 -translate-y-1/2 left-0 w-[140px] h-[26px] transition-all duration-500 ease-in-out ${
                isScrolled 
                  ? 'opacity-0 scale-95 translate-y-2 pointer-events-none' 
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 h-[38px] rounded-[40px] backdrop-blur-sm whitespace-nowrap"
            ...
            >
              ...
              <ChevronDown className="w-3 h-3" strokeWidth={1.5} color="white" />
            </button>
